...
class Diff{
  ...
  public static function compare(
    ...
    // only keep contents inside of <body></body> tags for custom soultion

    $string1 = preg_replace('/(.*?)<body(.*?)>/ims', '', $string1);
    $string2 = preg_replace('/(.*?)<body(.*?)>/ims', '', $string2);
    $string1 = preg_replace('/<\/body(.*?)html>/ims', '', $string1);
    $string2 = preg_replace('/<\/body(.*?)html>/ims', '', $string2);
    $string1 = preg_replace('/<\/body(.*?)>/ims', '', $string1);
    $string2 = preg_replace('/<\/body(.*?)>/ims', '', $string2);
...
$dom->loadHTMLFile($file1);
#$dom->formatOutput = true;
$body = $dom->documentElement->lastChild;
$file1 = $dom->saveHTML($body);
/*
$file1 = str_ireplace("<body>", "<body>\n", $file1);
$file1 = str_ireplace("</body>", "\n</body>", $file1);
*/
/*
$dom = new DOMDocument();
#$dom->preserveWhiteSpace = false;
$dom->loadHTMLFile($file2);
#$dom->formatOutput = true;
$body = $dom->documentElement->lastChild;
$file2 = $dom->saveHTML($body);
/*
$file2 = str_ireplace("<body>", "<body>\n", $file2);
$file2 = str_ireplace("</body>", "\n</body>", $file2);
*/
...
*/
  ...
  /* Returns a jason as a string:
  * line being the line number and type being del = delete, ins = insert and txt
  * being the line content e.g.
    [{
    "line" : "2", 
    "type"   : "ins",
    "txt" : "<p>this is some line text</p><br>",
    },
    {
    "line" : "1", 
    "type"   : "del",
  ...
  *              to "\n"
  */
  public static function toJson($diff){
    ...
    foreach ($diff as $line){
      ...
      switch ($line[1]){
        ...
        break;
        case self::DELETED  : $jsonDel[$jsoncountDel]['line'] = $count; $jsonDel[$jsoncountDel]['type'] = 'del';
        $jsoncountDel++;
        break;
        case self::INSERTED   : $jsonIns[$jsoncountIns]['line'] = $count; $jsonIns[$jsoncountIns]['type'] = 'ins'; $jsonIns[$jsoncountIns]['txt'] = $line[0]; ##utf8_encode($line[0]);
        $jsoncountIns++;
        break;
      }
    ...
    $string = json_encode(array_merge($jsonIns,array_reverse($jsonDel)));
